...
const steps = computed(() => {
  ...
  return [
    ...
    { key: 'accept', title: '接手订单', time: s === 'PENDING' ? '等待中…' : (o.locked_at ? fmt(o.locked_at) : '—'), active: ['LOCKED','DELIVERED','COMPLETED','DISPUTED'].includes(s), done: ['LOCKED','DELIVERED','COMPLETED','DISPUTED'].includes(s), pending: s === 'PENDING' },
...
<template>
  <section class="surface-card od-timeline-wrap p-4 sm:p-5" :class="{ 'shimmer-pending': order?.status === 'PENDING' }" aria-label="订单时间线">
    ...
    <p v-if="isBoostOrder && order?.status === 'PENDING'" class="mt-3 text-xs text-ink-3">打手接手订单后开始进行</p>
  </section>
</template>
